...
- (void)testCustomAttributeProcessingWithAppleConvertedSpaces
{
	...
	XCTAssertFalse(appleConvertedSpaceFound, @"There should be no custom class 'Apple-converted-space' as apple converted spaces are handled internally");
}
	...
	[string enumerateSubstringsInRange:entireStringRange options:NSStringEnumerationByParagraphs usingBlock:^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
		...
		switch (lineNumber)
		{
			case 1:
			{
				...
				XCTAssertEqual(numLists, (NSInteger)2, @"There should be two lists active on line 2, but %d found", numLists);
...
- (void)testPrefixWithNewlines
{
	...
	[string enumerateSubstringsInRange:entireStringRange options:NSStringEnumerationByParagraphs usingBlock:^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
		...
		XCTAssertFalse(foundNL, @"Newline in prefix of line %d", lineNumber);
...
- (void)testCorrectListBullets
{
	...
	[string enumerateSubstringsInRange:entireStringRange options:NSStringEnumerationByParagraphs usingBlock:^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
		...
		XCTAssertTrue(characterIsCorrect, @"Bullet Character on UL level %d should be '%@' but is '%@'", lineNumber+1, expectedChar, bulletChar);
...
- (void)testMixedListPrefix
{
	...
	[string enumerateSubstringsInRange:entireStringRange options:NSStringEnumerationByParagraphs usingBlock:^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
		...
		XCTAssertTrue(prefixIsCorrect, @"Prefix level %d should be '%@' but is '%@'", lineNumber+1, expectedPrefix, prefix);
...
- (void)testEmptyListItemWithSubList
{
	...
	[firstParagraphLists enumerateObjectsUsingBlock:^(DTCSSListStyle *oneList, NSUInteger idx, BOOL *stop) {
		...
		XCTAssertTrue(NSEqualRanges(commonRange, firstParagraphRange), @"List %d does not cover entire paragraph", idx+1);
	}];
	...
	[secondParagraphLists enumerateObjectsUsingBlock:^(DTCSSListStyle *oneList, NSUInteger idx, BOOL *stop) {
		...
		XCTAssertTrue(NSEqualRanges(commonRange, secondParagraphRange), @"List %d does not cover entire paragraph", idx+1);
	}];
